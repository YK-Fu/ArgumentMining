...
if __name__ == '__main__':
    ...
    raw['s'] = raw['s'].apply(lambda x: 'AGREE' if x == 1 else 'DISAGREE')
    
    raw['a'] = raw['s'] + ' [SEP] ' + raw['q'] + ' [SEP] ' + raw['r']
    raw['a'] = raw['a'].apply(lambda x: ' '.join(map(str, tokenizer(x, truncation=False, add_special_tokens=True)['input_ids'])))

    for k in [ 'q\'', 'r\'']:
        raw[k] = raw[k].apply(lambda x: ' '.join(map(str, tokenizer(x, truncation=False, add_special_tokens=False)['input_ids'])))
    ...
    for i, (a, q_, r_) in enumerate(zip(raw['a'], raw['q\''], raw['r\''])):
        q_s = a.find(q_)
        r_s = a.find(r_)
